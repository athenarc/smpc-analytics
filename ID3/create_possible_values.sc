...
uint64 createPossibleValues(string datasource, uint64 providers_vmap, uint64 data_providers_num) {
    ...
    for (uint64 j = 0 ; j < columns ; j++) {
        pd_shared3p float64[[1]] uniques(max_rows * data_providers_num); // initialize uniques with rows size
        uint64 unique_cnt = 0;
